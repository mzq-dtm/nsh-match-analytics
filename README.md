...
### 1.3 导入一场联赛数据
该部分操作建议在具有图形界面的电脑上进行，涉及图形界面的操作。
运行程序：
```
...
导入过程中如果发现数据库中缺少玩家数字 ID，会提示输入玩家 ID。
...
pip install -r requirements.txt
```
测试运行前，编辑 `config.py` 确保 `Config.DATABASE` 指向正确的数据库文件。测试运行
```
python app.py
```
如果成功启动，说明后端运行正常。
...
[Service]
Group=www-data
WorkingDirectory=/root/nsh/backend
Environment="PATH=/root/nsh/backend/venv/bin"
ExecStart=/root/nsh/backend/venv/bin/gunicorn \
...
cp -r /root/nsh/frontend/dist/* /var/www/nsh_frontend
