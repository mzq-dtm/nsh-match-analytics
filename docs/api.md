...
- 只返回 `player_id > 1` 的玩家；
- `nicknames` 只在同一 `player_id` 内去重：同一昵称对应多个有效期时，以该昵称最大的 `valid_from` 作为最近出现时间，再按最近出现时间从新到旧排列，最多返回三个不重复昵称；没有昵称历史时可能是空数组；
- 去重不跨 `player_id`：如果同一昵称出现在多个玩家的历史中，它可以同时出现在多个玩家项中；
...
成功：`200 OK`，返回以请求昵称为键的对象。当前时间点找不到有效昵称时，对应值为空数组。合服边界封口后，某玩家的首条合服后昵称有效期尚未由后续导入建立时，该玩家处于当前昵称空窗；即使 `/api/players` 仍能列出其历史昵称，本接口按这些昵称查询时也会返回空数组。
...
- `nicknames`：只在同一 `player_id` 内去重。同一昵称对应多个有效期时，以该昵称最小的 `valid_from` 作为首次出现时间，再按首次出现时间从旧到新用换行连接全部不重复昵称；去重不跨 `player_id`，同一昵称可以出现在多个玩家的返回项中；
