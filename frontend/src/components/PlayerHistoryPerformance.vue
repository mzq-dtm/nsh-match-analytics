    ...
    <div v-else-if="selectedPlayer" class="table-with-column-control">
      ...
      <div class="history-table-wrapper">
        <table class="results">
          ...
          <tbody>
            <tr v-for="(item,index) in sortedHistoryData" :key="item.match_id">
              <td>{{ index + 1 }}</td>
...
import { getPlayerHistory, getPlayers } from '@/api/nsh'
...
watch(searchText, (v) => {
  clearTimeout(_t)
  _t = setTimeout(() => { debouncedText.value = (v || '').trim() }, 200)
})
...
function highlightLabel(player) {
  ...
  // 1) 组合成一次性匹配（避免多次 replace 造成互相污染）
  const re = new RegExp(tokens
...
function selectPlayer(player) {
  selectedPlayer.value = player.player_id
  searchText.value = formatPlayerLabel(player)
  closeDropdown()
  onPlayerChange() // 复用你原有的拉取历史数据逻辑
}
...
columnsDef.forEach(col => visibleColumns[col.key] = true)
...
const formatPlayerLabel = player => `${player.nicknames.join(' / ')} (ID: ${player.player_id})`
...
async function fetchPlayers() {
  try {
    ...
    // 如果是从 MatchRecords 跳过来的，并且当前搜索框还是“ID: xxx”占位，就用昵称替换掉
...
async function onPlayerChange() {
  ...
  historyData.value = []
  try {
    const data = await getPlayerHistory(selectedPlayer.value)
...
function syncSearchLabelFromProps() {
  const id = props.externalPlayerId
  if (!id) return
  const p = players.value.find(x => String(x.player_id) === String(id))
  if (p) {
    // 找到了该玩家，用“昵称 (ID: xxx)”显示
    searchText.value = formatPlayerLabel(p)
  } else {
    // 还没拿到玩家列表或未匹配到，先用占位
...
// 外部跳转：当父组件传入 externalPlayerId 时，自动切换并拉取历史
watch(
